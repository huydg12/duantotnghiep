...
public interface BillRepository extends JpaRepository<Bill, Integer> {
  ...
  @Query("""
          ...
          JOIN bd.bill b
          WHERE b.status = com.poly.BE_main.utils.BillStatus.HOAN_TAT
            AND (:from IS NULL OR b.dateOfPayment >= :from)
            AND (:to IS NULL OR b.dateOfPayment <= :to)
            AND (:keyword IS NULL OR LOWER(p.productName) LIKE LOWER(CONCAT('%', :keyword, '%')))
          GROUP BY p.productName
          ORDER BY SUM(bd.quantity) DESC
      """)
  List<TopSellingProductDTO> getTopSellingProductsFiltered(
      @Param("from") LocalDate from,
      @Param("to") LocalDate to,
  ...
  @Query("""
      ...
      AND (:from IS NULL OR b.dateOfPayment >= :from)
      AND (:to IS NULL OR b.dateOfPayment <= :to)
                              AND (:keyword IS NULL OR LOWER(p.brand.name) LIKE LOWER(CONCAT('%', :keyword, '%')))
                            GROUP BY p.brand.name
                        """)
  List<RevenueByBrandDTO> getRevenueByBrandFiltered(
      @Param("from") LocalDate from,
      @Param("to") LocalDate to,
      @Param("keyword") String keyword);
