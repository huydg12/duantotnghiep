    ...
    async function resetPassword(id) {
        ...
        try {
            await axios.put(`http://localhost:8080/account/updatePassword/${id}`, updatePassword)
            alert('Cập nhật mật khẩu thành công');
            await fetchAccount();
        } catch (error) {
            console.error('Lỗi cập nhật mật khẩu:', error.response ? error.response.data : error.message);
